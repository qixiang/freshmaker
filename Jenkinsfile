...
try { // massive try{} catch{} around the entire build for failure notifications
    ...
    //     }
    //     else if (branch ==~ /[0-9]+/) {
    //         def prLink = "https://github.com/redhat-exd-rebuilds/freshmaker/pull/${branch}"
    //         echo "Building PR #${branch}: ${prLink}"
    //         currentBuild.displayName = "PR #${branch}"
    //         currentBuild.description = prLink
    //     }
    // }
    timestamps {
        node('docker') {
    ...
    if (branch ==~ /[0-9]+/){
        BODY = BODY + "\nPull Request: https://github.com/redhat-exd-rebuilds/freshmaker/pull/${branch}"
    }
